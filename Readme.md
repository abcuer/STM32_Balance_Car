...
- **蓝牙模式**：点亮蓝灯表示进入蓝牙模式。在此模式下，可通过手机控制小车移动；当小车距离障碍物小于20cm时，小车停止运动，并触发声光报警；当障碍物远离至50cm以外时，恢复蓝牙控制功能。
- **跟随模式**：点亮红灯表示进入跟随模式。小车会自动跟随前方30cm范围内的物体；若物体距离小于30cm，小车将自动后退，以保持与物体30cm的安全距离。
- **拿起检测**：当小车被拿起并持续超过1秒时，小车将自动停止运行。
- **放下检测**：当小车处于直立状态（Pitch角度在正常范围内），并且放置时间超过1秒时，小车将自动恢复运行。
